...
namespace Ictshop.Controllers
{
    public class GioHangController : Controller
    {
        ...
        public ActionResult GioHang()
        {
            ...
            {
                Sanpham sp = db.Sanphams.SingleOrDefault(n => n.Masp == item.iMasp);
                GioHang sanpham = lstGioHang.SingleOrDefault(n => n.iMasp == item.iMasp);
                //Nếu mà tồn tại thì chúng ta cho sửa số lượng
                if (sanpham != null)
                {
                    if (sanpham.iSoLuong > sp.Soluong)
                    {
                        ViewBag.Loi = "Số lượng sản phẩm trong kho không đủ!";
                    }                    
                }                            
            }
        ...
        public ActionResult DatHang(FormCollection donhangForm)
        {
            ...
            db.SaveChanges();
            if(ddh.maThanhtoan == 1)
            {
                return RedirectToAction("Index", "Donhangs");
            }
            return RedirectToAction("Payment", new { @id = ddh.Madon });
        }
        #endregion
        ...
        {
            var other = db.Donhangs.FirstOrDefault(x => x.Madon == id);
            ...
            pay.AddRequestData("vnp_Amount", (other.Tongtien * 100).ToString()); //số tiền cần thanh toán, công thức: số tiền * 100 - ví dụ 10.000 (mười nghìn đồng) --> 1000000
            ...
            pay.AddRequestData("vnp_OrderInfo", "Thanh toán đơn hàng: " + (other.Madon).ToString()); //Thông tin mô tả nội dung thanh toán
            pay.AddRequestData("vnp_OrderType", "other"); //topup: Nạp tiền điện thoại - billpayment: Thanh toán hóa đơn - fashion: Thời trang - other: Thanh toán trực tuyến
            pay.AddRequestData("vnp_ReturnUrl", returnUrl); //URL thông báo kết quả giao dịch khi Khách hàng kết thúc thanh toán
            pay.AddRequestData("vnp_TxnRef", (other.Madon).ToString()); //mã hóa đơn
